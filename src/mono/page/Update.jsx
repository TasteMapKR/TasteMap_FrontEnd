...
import { fetchCourseData, updateCourse } from '../api/api'; // api.js에서 가져오기
import { useParams, useNavigate } from 'react-router-dom';
import '../page/create/Create.css'; // 같은 CSS 파일 사용
...
const Update = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const { course, roots } = await fetchCourseData(id, accessToken);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const accessToken = localStorage.getItem('Access');
            await updateCourse(id, formData, accessToken);
            navigate(`/course/${id}`);
        } catch (error) {
            if (error.message.includes('Failed to update course')) {
                const validationErrors = JSON.parse(error.message).errors;
                Object.keys(validationErrors).forEach(field => {
                    setError(field, { type: 'manual', message: validationErrors[field] });
                });
            } else {
                console.error('코스 업데이트 중 오류 발생:', error.message);
    ...
    return (
        <form className="create-container" onSubmit={handleSubmit(onSubmit)}>
